...
import { useGame, type AnimationSet } from "./stores/useGame.ts";
import React from "react";

export function EcctrlAnimation(props: EcctrlAnimationProps) {
  // Change the character src to yours
  const group = useRef();
  const { actions, names } = useAnimations(props.animations, group);
  ...
  useEffect(() => {
    ...
    const action =
      actions[
        curAnimation ? curAnimation : props.animationSet.idle?.toString()
      ];
