...
import { useMouseCapture } from "../../../hooks/useMouseCapture.ts";
...
export const useFollowCam = function (props: UseFollowCamProps) {
  ...
  // let rayLength = null;
  // let rayHit = null;
  const { isPointerLocked } = useMouseCapture();
  ...
  const onTouchMove = (e: TouchEvent) => {
    ...
    if (previousTouch1 && !previousTouch2) {
      ...
      const vy =
        followCam.rotation.x + touch1MovementY * 0.005 * (camMoveSpeed || 1);

      cameraDistance = followCam.position.length();

      if (vy >= (camLowLimit || 0) && vy <= (camUpLimit || 0)) {
        followCam.rotation.x = vy;
        followCam.position.y = -cameraDistance * Math.sin(-vy);
        followCam.position.z = -cameraDistance * Math.cos(-vy);
      }
  ...
  const joystickCamMove = (movementX: number, movementY: number) => {
    pivot.rotation.y -= movementX * 0.005 * (camMoveSpeed || 1) * 3;
    const vy =
      followCam.rotation.x + movementY * 0.005 * (camMoveSpeed || 1) * 3;
  ...
  useEffect(() => {
    ...
    if (camListenerTarget === "domElement") {
      element.addEventListener("mousemove", onDocumentMouseMove);
    ...
    return () => {
      if (camListenerTarget === "domElement") {
        element.removeEventListener("mousemove", onDocumentMouseMove);
